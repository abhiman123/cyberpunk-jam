...
// â”€â”€ Procedural generator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function finalizeTile(cell) {
    ...
    // count === 1: dead-end stub â€” use a straight that covers the direction
    cell.type = 'straight';
...
export default class CircuitRouting extends MinigameBase {
    ...
    _build(caseData) {
        ...
        // Backdrop â€” diegetic hologram panel
        ...
        const title = this.scene.add.text(640, 85, 'CIRCUIT DIAGNOSTIC â€” ROUTE POWER TO OUTPUTS', {
        ...
        this._outputSpecs.forEach((outputSpec) => {
            ...
            const lbl = this.scene.add.text(ox + 16, oy - 7, outputSpec.label, {
                fontFamily: 'monospace', fontSize: '11px', color: requiredPalette.idle,
        ...
        // Close button â€” added to the container LAST so it sits on top of all tiles
    ...
    _buildRepairTargetPanel(depth) {
        ...
        this._repairTargetViews = this._repairTargets.map((target, index) => {
            ...
            const statusText = this.scene.add.text(-72, y + 9, target.powerClass && target.powerClass !== 'neutral'
                ...
                : 'BROKEN', {
                fontFamily: 'monospace', fontSize: '9px', color: target.powerClass && target.powerClass !== 'neutral' ? requirementPalette.label : '#ffb695',
            }).setOrigin(0, 0.5);
    ...
    _buildTile(x, y, depth) {
        ...
        // A square bg looks identical at every 90Â° step so it never needs to rotate.
    ...
    _animateTileRotation(tileView, targetRotation) {
        ...
        // Snap to the previous clean step so the tween always covers exactly 90Â°,
