...
export class RulebookOverlay {
    ...
    setRuleState(allRules, activeRuleIds) {
        this.allRules = Array.isArray(allRules) ? allRules : [];
        this.activeRuleIds = Array.isArray(activeRuleIds) ? activeRuleIds : [];
        if (this._visible) {
            this._refresh();
    ...
    _build() {
        ...
        this._contentContainer = this.scene.add.container(this._viewportX + 18, this._viewportY + 18);

        this._scrollTrack = this.scene.add.rectangle(this._panelX + PANEL_W - 18 - 10, this._viewportY + (this._viewportH / 2), 6, this._viewportH - 18, 0x0e1a21, 1)
            .setStrokeStyle(1, 0x28424d, 0.68);
        this._scrollThumb = this.scene.add.rectangle(this._panelX + PANEL_W - 18 - 10, this._viewportY + 34, 6, 86, 0x89eaff, 0.92)
            .setVisible(false);
        ...
        this._contentMaskSource = this.scene.make.graphics({ x: 0, y: 0, add: false });
        this._contentMaskSource.fillStyle(0xffffff, 1);
        this._contentMaskSource.fillRect(this._viewportX, this._viewportY, this._viewportW, this._viewportH);
        this._contentMask = this._contentMaskSource.createGeometryMask();
        this._contentContainer.setMask(this._contentMask);
    ...
    _refresh() {
        ...
        this._headerSubtitle.setText('ONE RULE // ONE IMAGE // SCROLL OR DRAG');

        const contentHeight = this._renderContent(copy, sectionInfo, getPreviewDay(this._selectedDay));
        this._scrollMax = Math.max(0, contentHeight - (this._viewportH - 36));
        this._setScrollOffset(this._scrollOffset, true);
    }

    _renderContent(copy, sectionInfo, previewDay) {
        const contentWidth = this._viewportW - 56;
        const leftWidth = 248;
        const columnGap = 20;
        const rightWidth = contentWidth - leftWidth - columnGap;
        let y = 0;

        const topBadge = this._addContentNode(this.scene.add.rectangle(86, 14, 172, 28, 0x102934, 1)
            .setStrokeStyle(1, sectionInfo.accent, 0.78));
        const topBadgeLabel = this._addContentNode(this.scene.add.text(86, 14, 'QUICK REFERENCE', {
            fontFamily: 'Courier New',
            fontSize: '11px',
        ...
        const ruleLabel = this._addContentNode(this.scene.add.text(20, columnTop + 16, 'RULE', {
            fontFamily: 'Courier New',
            fontSize: '11px',
            color: colorToCss(sectionInfo.accent),
            letterSpacing: 2,
        }).setOrigin(0, 0));
        const ruleText = this._addContentNode(this.scene.add.text(20, columnTop + 38, copy.rule, {
            fontFamily: 'Courier New',
            fontSize: '22px',
            color: '#effcff',
            wordWrap: { width: leftWidth - 40 },
            lineSpacing: 6,
        }).setOrigin(0, 0));

        const captionLabel = this._addContentNode(this.scene.add.text(20, columnTop + 112, 'SCRAP SIGNAL', {
            fontFamily: 'Courier New',
            fontSize: '10px',
            color: '#86c7d8',
            letterSpacing: 2,
        }).setOrigin(0, 0));
        const captionText = this._addContentNode(this.scene.add.text(20, columnTop + 132, copy.caption, {
            fontFamily: 'Courier New',
            fontSize: '16px',
            color: '#d0ecf3',
            wordWrap: { width: leftWidth - 40 },
        ...
        const footerText = this._addContentNode(this.scene.add.text(22, y + 36, 'Use the tab picture as the fast read. The left note is the whole call.', {
            fontFamily: 'Courier New',
            fontSize: '15px',
            color: '#d0ecf3',
        ...
        const hint = this._addContentNode(this.scene.add.text(8, y, 'WHEEL TO SCROLL // DRAG THE PICTURE // CLICK OUTSIDE TO CLOSE', {
            fontFamily: 'Courier New',
            fontSize: '10px',
            color: '#6e93a0',
            letterSpacing: 1,
    ...
    _createPreviewCard(x, y, width, height, sectionInfo, previewDay) {
        ...
        const label = this._addContentNode(this.scene.add.text(x + 20, y + 14, 'REFERENCE IMAGE', {
            fontFamily: 'Courier New',
            fontSize: '10px',
            color: sectionInfo.text,
            letterSpacing: 2,
        ...
        return { frame, inset, label, previewStack };
    }
    ...
    _drawOverviewPreview(container, size, palette) {
        ...
        const stripText = this.scene.add.text(0, -112, 'PICK A TAB. LEARN ONE RULE. MOVE.', {
            fontFamily: 'Courier New',
            fontSize: '11px',
        ...
        cards.forEach((card) => {
            ...
            const sub = this.scene.add.text(card.x, card.y + 10, 'ONE RULE // ONE IMAGE', {
                fontFamily: 'Courier New',
                fontSize: '8px',
                color: palette.accentText,
                letterSpacing: 1,
    ...
    _drawGridPreview(container, size, palette) {
        ...
        this._drawCircuitPipPattern(domino, 1, dominoX, -36, palette.monochrome ? 0xffffff : 0x12351f, palette.monochrome ? 0.84 : 1);
        this._drawCircuitPipPattern(domino, 4, dominoX, 36, palette.monochrome ? 0xffffff : 0x12351f, palette.monochrome ? 0.84 : 1);

        const note = this.scene.add.text(102, 92, 'LIKE THE LIVE TILE + REAL DOMINO FACE', {
            fontFamily: 'Courier New',
            fontSize: '9px',
            color: palette.accentText,
            align: 'center',
    ...
    _drawCircuitPipPattern(graphics, value, centerX, centerY, color, alpha = 1) {
        ...
        if (value === 1) {
            drawTrace(centerX - 8, centerY - 14, centerX - 8, centerY + 14);
            drawTrace(centerX + 8, centerY - 14, centerX + 8, centerY + 14);
            return;
        }
        if (value === 2) {
            drawRing(centerX, centerY - 12);
            drawRing(centerX, centerY + 12);
            return;
        }
        if (value === 3) {
            drawTrace(centerX - 16, centerY - 10, centerX + 16, centerY - 10);
            drawTrace(centerX - 12, centerY, centerX + 12, centerY);
            drawTrace(centerX - 8, centerY + 10, centerX + 8, centerY + 10);
            return;
        }
        if (value === 4) {
            drawRing(centerX - 12, centerY - 12);
            drawRing(centerX + 12, centerY - 12);
            drawRing(centerX - 12, centerY + 12);
            drawRing(centerX + 12, centerY + 12);
        }
    ...
    _setScrollOffset(nextOffset, force = false) {
        ...
        this._contentContainer.y = (this._viewportY + 18) - clampedOffset;
    ...
    _refreshScrollVisuals() {
        ...
        const trackHeight = this._viewportH - 18;
        ...
        this._scrollThumb.y = this._viewportY + 9 + (thumbHeight / 2) + (travel * progress);
    }
